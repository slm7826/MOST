...
EOF
        ...
        var=flux_t
        $tooldir/plot.py $commonFlags --y=$var --ylim=-500:2500 --save "$outdir/$var.pdf" $outdir/tmp/*.csv

        var=b_star
        $tooldir/plot.py $commonFlags --y=$var --ylim=-0.05:0.5 --save "$outdir/$var.pdf" $outdir/tmp/*.csv

        var=flux_m
        $tooldir/plot.py $commonFlags --y=$var --ylim=-0.1:4.5  --save "$outdir/$var.pdf" $outdir/tmp/*.csv

        var=u_star
        $tooldir/plot.py $commonFlags --y=$var --ylim=-0.01:2.0 --save "$outdir/$var.pdf" $outdir/tmp/*.csv
    done
done
